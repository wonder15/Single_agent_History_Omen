    ...
    while True:
        ...
        query GetHistory($user: String!, $maxTime: BigInt!, $minTime: BigInt!) {
            fpmmTrades(
                ...
                id                  # <--- Matches your requested field congruent with subgraph
                creationTimestamp
                type
                collateralAmount
                feeAmount
...
def process_trades(trades):
    ...
    for t in trades:
        ...
        # If you want the RAW full string 100%, remove the slicing below.
    ...
    print("-" * 130)
    print(f"TOTALS: {'':<41} | ${total_cost:<8.2f} | ${total_rev:<8.2f}")
    if total_cost > 0:
